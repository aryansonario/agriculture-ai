// Use relative paths for API calls - works on both local dev and Vercel production
const FETCH_INTERVAL = 10000;

// For local development with Flask running on port 5000, you may need to set this:
// const FLASK_URL = "http://localhost:5000";
// For Vercel production, use relative paths (both API and frontend served from same origin)
const FLASK_URL = "";  // Empty string means use relative paths from current origin
...
function buildApiUrl(path) {
  ...
  return path;  // Relative path for same-origin requests
}
...
async function fetchLatestAdvice(data) {
  ...
  try {
    const response = await fetch(buildApiUrl("/api/advice/latest"));
...
async function saveFieldContext(event) {
  ...
  try {
    const response = await fetch(buildApiUrl("/api/farm-context"), {
      method: "POST",
...
els.pumpToggleBtn.addEventListener("click", async () => {
  ...
  try {
    await fetch(buildApiUrl("/api/pump"), {
      method: "POST",
      headers: { "Content-Type": "application/json" },
